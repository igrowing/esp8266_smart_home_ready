...
boolean Adafruit_BMP085::begin(uint8_t mode) {
  ...
  write8(0xE0, 0xB6);
  uint32_t ts = millis();
  while (millis() < ts + 3) ;  // wait to complete reset
...
uint16_t Adafruit_BMP085::readRawTemperature(void) {
  ...
  delay(5);
  uint16_t t = read16(BMP085_TEMPDATA);
#if BMP085_DEBUG == 1
  Serial.print(">> Raw temp: "); Serial.println(t);
#endif
  return t;
}
...
int32_t Adafruit_BMP085::readPressure(void) {
  int32_t UT, UP, B3, B5, B6, X1, X2, X3, p = 0;
  uint32_t B4, B7 = 0;
...
#if BMP085_DEBUG == 1
  Serial.print("final p = "); Serial.println(p);
#endif
  return p;
}
...
// #endif
  ...
  temp /= 10;
#if BMP085_DEBUG == 1
  Serial.print("raw_t = "); Serial.println(UT);
